...
Cretop에 직접 로그인한 뒤, 화면에 열린 조건검색 결과 테이블을 CSV로 저장하는 데스크탑 도구입니다.
...
2. 열린 Chrome에서 Cretop에 직접 로그인합니다.
3. Cretop에서 조건검색을 직접 실행하고, 결과 테이블이 보이는 화면까지 이동합니다.
4. 앱으로 돌아와 `로그인 완료`를 누릅니다.
5. 왼쪽 메뉴에서 `조건검색 복사`를 엽니다.
...
기본 저장 파일은 앱 데이터 폴더 아래의 `output/cretop_condition_search.csv`입니다.

다른 위치에 저장하려면 `조건검색 복사` 화면에서 `변경`을 누르고 CSV 파일 경로를 선택하세요.
...
- 엑셀 파일을 읽어 Cretop 검색을 자동으로 반복 실행하는 기능
